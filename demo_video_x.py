...
print(f"Model class: {hybrik_model.__class__}")
...
for img_path in tqdm(img_path_list):
    ...
    with torch.no_grad():
        ...
        if opt.save_pk:
            ...
            pred_theta = pose_output.pred_theta_mat.squeeze(0).cpu().numpy()
            pred_beta = pose_output.pred_beta.squeeze(0).cpu().data.numpy()
            pred_phi = pose_output.pred_phi.squeeze(0).cpu().data.numpy()

            res_db['pred_beta'].append(pred_beta)
            res_db['pred_phi'].append(pred_phi)
            res_db['pred_theta_mat'].append(pred_theta)
